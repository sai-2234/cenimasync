...
const CHAT_THEMES = [
    ...
    { id: 'nebula',    name: 'Nebula Aura',         preview: 'linear-gradient(135deg,#B24CFF,#FF4CD1)' },
    { id: 'matrix',    name: 'Matrix',              preview: 'linear-gradient(135deg,#00FF9C,#00B368)' },
    { id: 'royal',     name: 'Royal Velvet',        preview: 'linear-gradient(135deg,#4B3AFF,#1B0F6B)' }
];
...
function resolveInitialRoute(){
    ...
    if (pendingAuthUser){
        // state.profile may still be mid-fetch if auth resolved just before
        // this fires — in that case do nothing here; the onAuthStateChanged
        // callback below will call enterHome() itself once the profile load
        // completes (splashDone will already be true by then).
        if (state.profile) enterHome();
    } else {
        showScreen('auth');
    }
}
...
function subscribeChats(){
    if (!state.user) return; // never query chats before the auth session is confirmed
...
$('#avatar-file-input')?.addEventListener('change', e => {
    ...
    // Hard safety net — the spinner can never hang forever, no matter what
    // stage silently stalls (network, rules, a dropped 'complete' event).
    const safetyTimer = setTimeout(() => {
        if (!state.avatarUploading) return;
        stopSpinner();
        toast('Upload timed out — check your connection and try again', 'error');
    }, 30000);
    ...
    task.on('state_changed',
        ...
        () => {
            getDownloadURL(task.snapshot.ref)
                .then(url => updateProfile(auth.currentUser, { photoURL: url }).then(() => url))
                .then(url => updateDoc(doc(db, 'users', state.user.uid), { photoURL: url }).then(() => url))
                .then(url => {
                    // Reflect the change everywhere it's currently visible,
                    // instantly — peers see it live via their own
                    // peerUnsub/sidebar-row listeners.
